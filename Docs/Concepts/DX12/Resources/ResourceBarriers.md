...
GPU 하드웨어는 리소스를 어떤 용도로 쓰느냐에 따라 내부적으로 캐시, 압축, 메모리 레이아웃을 다르게 관리합니다. 예를 들어 렌더 타겟으로 쓰이던 텍스처를 셰이더에서 읽으려면 렌더 타겟 전용 캐시를 플러시하고 압축을 해제해야 할 수 있습니다. 배리어는 이런 전환을 드라이버/GPU에 알리는 역할을 합니다.
